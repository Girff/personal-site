...
def main():
    ...
    # also write the data as a script file so the site works on file://
    # (browsers block fetch() there, but <script src> is fine)
    js_path = OUT_PATH[:-len(".json")] + ".js"
    with open(js_path, "w", encoding="utf-8") as f:
        f.write("/* generated by scripts/fetch_letterboxd.py — do not edit */\n")
        f.write("window.LETTERBOXD_DATA = ")
        json.dump(data, f, ensure_ascii=False)
        f.write(";\n")
    print(f"wrote {os.path.normpath(js_path)}")
